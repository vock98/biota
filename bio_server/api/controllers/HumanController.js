...
module.exports = {   
    ...
    human_add:function(req,res){
        var return_obj = {};
        return_obj.link_to_submit = "/api/Ds_human";  
        return_obj.link_to_list = "/human/list/"; 
        return_obj.now_url = "基本資料維護";  
        return res.view( false , return_obj );                               		
	},
    //編輯頁面 http://localhost:1337/human/edit/:id
    ...
    //刪除頁面 有人同一時間改姓名會造成瞬間刪除失敗 (客戶需求) http://localhost:1337/human/delete/:id/:ds_name
    human_delete:function(req,res){        
        var params = req.allParams();        
        var destroy_url = "/api/Ds_human?type=D&id="+params.ds_human_pk+"&name="+params.ds_name+"&submit_to_link=/human/list/";
        return res.redirect( destroy_url );
	},
    //進出頁面 http://localhost:1337/human/inout
