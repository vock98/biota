...
module.exports = {
    ...
    find_R1_db: function( search_cond, who ){
        return new Promise(function(resolve, reject){
            Ds_human.find( search_cond ).populateAll().exec(function(err,find_data){
                if(err){
            ...
            Ds_human.update( search_cond, update_cond ).exec(function(err,update_data){
                if(err){
                    ...
                    if( _.isEmpty(update_data) ){
                        ...
                        //為了要populate重撈一次
                        Ds_human.findOne( {ds_human_pk: update_data[0].ds_human_pk} ).populateAll().exec(function(err,find_data){
                            if(err){
                                no_call_service.write_log(table_name,"U_die", err, who, log_type);
                                var return_data = no_call_service.add_biota_result( {}, false, err.details, "");
                                resolve(return_data);
        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    ...
                    var final_data = yield human_service.write_db( r_array[0], who );
        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    ...
                    var final_data = yield human_service.find_R1_db( r_array[0], who );
                    var back_data =  yield call_service.back_change_cond(final_data, change_obj);
                    resolve( back_data );
                }else{
                    //輸入條件有誤 直接回傳錯誤JSON
        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    ...
                    var r_array =  yield call_service.check_change_cond(input_params, change_obj, cond_array);
                    r_array[0].ds_deleted = {"$exists":false}; //補上刪除不可被查
                    var final_data = yield human_service.find_R2_db( r_array[0], who );
                    var back_data =  yield call_service.back_change_cond(final_data, change_obj);
                    resolve( back_data );
                }else{
                    //輸入條件有誤 直接回傳錯誤JSON
        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    ...
                    var final_data = yield human_service.update_db( r_array[1], r_array[0], who );
                    var back_data =  yield call_service.back_change_cond(final_data, change_obj);
                    resolve( back_data );
                }else{
                    //輸入條件有誤 直接回傳錯誤JSON
        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    //輸入條件正確 修正資料ID內容
                    var r_array =  yield call_service.check_change_cond(input_params, change_obj, cond_array);
                    var final_data = yield human_service.destroy_db( r_array[0], who );
                    var back_data =  yield call_service.back_change_cond(final_data, change_obj);
                    resolve( back_data );
                }else{
                    //輸入條件有誤 直接回傳錯誤JSON
