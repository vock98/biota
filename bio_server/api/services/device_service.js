        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    ...
                    var final_data = yield device_service.write_db( r_array[0], who );
        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    ...
                    var final_data = yield device_service.find_R_db( r_array[0], who );
                    var back_data =  yield call_service.back_change_cond(final_data, change_obj);
                    resolve( back_data );
                }else{
                    //輸入條件有誤 直接回傳錯誤JSON
        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    ...
                    var final_data = yield device_service.update_db( r_array[1], r_array[0], who );
                    var back_data =  yield call_service.back_change_cond(final_data, change_obj);
                    resolve( back_data );
                }else{
                    //輸入條件有誤 直接回傳錯誤JSON
        ...
        return new Promise(function(resolve, reject){
            co(function* () {                                                    
                ...
                if(check_fill_nfill == "ok"){
                    //輸入條件正確 修正資料ID內容
                    var r_array =  yield call_service.check_change_cond(input_params, change_obj, cond_array);
                    var final_data = yield device_service.destroy_db( r_array[0], who );
                    var back_data =  yield call_service.back_change_cond(final_data, change_obj);
                    resolve( back_data );
                }else{
                    //輸入條件有誤 直接回傳錯誤JSON
